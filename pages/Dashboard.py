...
if not df.empty:
    ...
    total_posts = positive = negative = neutral = 0
...
if total_posts > 0:
    positive_percentage = round((positive / total_posts) * 100, 2)
...
with right:
    ...
    st.success("🟢 AI Sentiment Model Active")
...
if not df.empty:
    show_columns = ["text", "sentiment", "confidence"]
    st.dataframe(df[show_columns].tail(5), use_container_width=True)
else:
